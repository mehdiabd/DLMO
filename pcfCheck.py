...
print("Grouping PCF state-errors ...")

if __name__ == '__main__':
    for ip in pilot_ips:
        res = dlm_db['profile_configuration_function'].aggregate(
            [
                {"$match": {"sdate": get_short_date(), "ip_adr": ip}},
                {"$project": {"state": 1, "error": 1}},
                {
                    "$group":
                        {
                            "_id": {"stat": "$state", "erro": "$error"}
                            # ,"SEpair": {"$push": {"state": "$state", "error": "$error"}}
                        }
                }
            ]
        )
